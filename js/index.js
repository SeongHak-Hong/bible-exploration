...
for (i = 0; i < menuLi.length; i++) {
  menuLi[i].addEventListener('click', function(event) {
    if (event.target.id == 'home-btn') // 홈 버튼 클릭시
      ...
      window.scrollTo({left: 0, top: getSectionHeight * 4, behavior: 'smooth'});
  });

  // sec[i].addEventListener('wheel', function (event) {
  //   // 휠 기본기능 막기
  ...
  sec[i].addEventListener('wheel', function (event) {
    ...
    if (event.deltaY > 0) {
      ...
      if (order < sec.length) {
        currentHeightValue = currentHeightValue + (getSectionHeight);
        // 현재값은 ->  현재값 + 자식높이값이다.
