  ...
  for (i = 0; i < menuLi.length; i++) {
    menuLi[i].addEventListener('click', function(event) {
      if (event.target.id == 'home-btn') // 홈 버튼 클릭시
        ...
        window.scrollTo({left: 0, top: (getBodyHeight / 6) * 1, behavior: 'smooth'});
      else if (event.target.id == 'ani-btn') //애니메이션 버튼 클릭시
        window.scrollTo({left: 0, top: (getBodyHeight / 6) * 2, behavior: 'smooth'});
      else if (event.target.id == 'notice-btn') //안내문 버튼 클릭시
        window.scrollTo({left: 0, top: (getBodyHeight / 6) * 3, behavior: 'smooth'});
      else if (event.target.id == 'contest-btn') //전시 버튼 클릭시
        window.scrollTo({left: 0, top: (getBodyHeight / 6) * 4, behavior: 'smooth'});
